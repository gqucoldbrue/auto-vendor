...
var RuleProcessor = class {
  ...
  getParamsOrThrow() {
    ...
    if (!isReady) {
      throw new import_shared4.PandaError("MISSING_PARAMS", "RuleProcessor is missing params, please call `clone` first");
    }
    return {
...
var FileMatcher = class {
  ...
  createMatch = (mod, keys2) => {
    ...
    return (0, import_shared5.memo)((id) => {
      return !!mods.find((mod2) => {
        if (mod2.kind === "namespace") {
          return keys2.includes(id);
        }
        return mod2.alias === id || mod2.name === id;
      });
    });
  };
  match = (id) => {
  ...
  isValidPattern = (id) => {
    ...
    const match3 = this._patternsMatcher;
    return match3(id);
  };
  _recipesMatcher;
  isValidRecipe = (id) => {
    if (!this._recipesMatcher)
      this._recipesMatcher = this.createMatch(this.importMap.recipe, this.context.recipes.keys);
    const match3 = this._recipesMatcher;
    return match3(id);
  };
  isRawFn = (fnName) => {
  ...
  matchFn = (0, import_shared5.memo)((fnName) => {
    ...
    if (ns) {
      if (ns.mod.includes(this.importMap.css) && cssEntrypointFns.has(identifier))
        return true;
      if (ns.mod.includes(this.importMap.recipe) && this.recipeAliases.has(identifier))
        return true;
      if (ns.mod.includes(this.importMap.pattern) && this.patternAliases.has(identifier))
        return true;
      return this.functions.has(identifier);
    }
    return false;
  });
  isJsxFactory = (0, import_shared5.memo)((tagName) => {
...
var ImportMap = class {
  constructor(context) {
    ...
    const importMap = (0, import_shared6.mapEntries)(this.normalize(context.config.importMap), (key, paths) => [key, paths.join("/")]);
...
var JsxEngine = class {
  ...
  get upperName() {
    return (0, import_shared7.capitalize)(this.jsxFactory);
  }
  get typeName() {
    return `HTML${(0, import_shared7.capitalize)(this.jsxFactory)}Props`;
  }
  get variantName() {
    return `${(0, import_shared7.capitalize)(this.jsxFactory)}VariantProps`;
  }
  get componentName() {
    return `${(0, import_shared7.capitalize)(this.jsxFactory)}Component`;
  }
  isJsxFactory = (name) => {
    return name === this.factoryName;
  };
  isJsxTagRecipe = (0, import_shared7.memo)((tagName) => {
    return this.recipeMatcher.string.has(tagName) || this.recipeMatcher.regex.some((regex) => regex.test(tagName));
  });
  isJsxTagPattern = (0, import_shared7.memo)((tagName) => {
    return this.patternMatcher.string.has(tagName) || this.patternMatcher.regex.some((regex) => regex.test(tagName));
  });
  isRecipeOrPatternProp = (0, import_shared7.memo)((tagName, propName) => {
...
var Layers = class {
  ...
  getLayerRoot(layer) {
    ...
    switch (layer) {
      ...
      default:
        throw new import_shared8.PandaError("INVALID_LAYER", `Unknown layer: ${layer}`);
    }
  }
  insert() {
    const { root } = this;
...
var Patterns = class {
  ...
  createDetail(name, pattern) {
    ...
    return {
      ...
      config: pattern,
      type: "pattern",
      match: (0, import_shared9.createRegex)(jsx),
      jsx
    };
  }
  getConfig(name) {
  ...
  transform(name, styles) {
    ...
    const _styles = (0, import_shared9.getPatternStyles)(pattern, styles);
  ...
  find = (0, import_shared9.memo)((jsxName) => {
    return this.details.find((node) => node.match.test(jsxName))?.baseName ?? (0, import_shared9.uncapitalize)(jsxName);
  });
  ...
  static isValidNode = (node) => {
    return (0, import_shared9.isObject)(node) && "type" in node && node.type === "recipe";
  };
};
...
Object.keys(keys).forEach((key) => {
  ...
  unitlessProperties.add((0, import_shared10.hypenateProperty)(key));
});
...
function stringify(value) {
  ...
  const write = (cssText, selectors, conditions, name, data, isAtRuleLike, isVariableLike) => {
    ...
    if (isAtRuleLike) {
      ...
      name = `${(0, import_shared11.hypenateProperty)(name)}: `;
    }
